...
const TestimonialCard = ({ data }) => {
  ...
  return (
    <div
      className="group relative bg-[#0a0a0a] border border-white/[0.08] rounded-[1.8rem] md:rounded-[2.5rem] overflow-hidden transition-all duration-500 hover:scale-[1.02] hover:border-white/20 hover:shadow-[0_20px_50px_rgba(0,0,0,0.8)] cursor-pointer"
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
      onClick={handleMobileClick}
    >
      {/* Container: Added min-height for mobile safety */}
      <div className="relative aspect-[9/16] w-full min-h-[350px] md:min-h-0 overflow-hidden bg-[#111] rounded-t-[1.8rem] md:rounded-t-[2.5rem]">
        <video
          ref={videoRef}
          src={data.mediaUrl}
          className="w-full h-full object-cover"
          muted
          playsInline // Essential for iOS/Android
          loop
          preload="metadata" // Ensures video loads its first frame
        />

        <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent pointer-events-none" />

         <div className="absolute bottom-4 right-4 bg-black/50 p-2 rounded-full pointer-events-none opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
           <span className="text-white text-[10px] md:text-xs">
             {isMobile ? "Tap to Play" : "Hover for Preview"}
           </span>
        </div>
      </div>
      ...
      <div className="p-5 md:p-6 bg-black">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-gray-500 text-xs md:text-sm leading-snug line-clamp-2">
            {data.message}
          </p>
          <div className="flex items-center gap-2 mt-3">
            <span className="text-[#a855f7] text-xs">◆</span>
            <span className="text-gray-400 text-[10px] md:text-xs font-semibold tracking-widest">
              {data.rating} / 5 RATING
            </span>
          </div>
        </div>
