...
const CollectInstallment = () => {
  ...
  const batch = user?.courses?.find(c => c._id?.toString() === batchId);
  const studentEntry = batch?.studentEnrolled?.find(
    (entry) => entry.student?._id?.toString() === studentId
  );
  ...
  if (!enrollment || !student) {
    return (
      <div className="min-h-screen bg-[#09090b] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <Spinner />
          <p className="text-emerald-500 text-sm">Loading Details...</p>
        </div>
      </div>
    );
  }
  ...
  const handleSubmit = () => {
    ...
    if (numericAmount > remaining) return alert("Amount exceeds remaining");
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white flex justify-center items-center px-3 sm:px-4 py-6">
      
      <div className="bg-[#18181b]/80 backdrop-blur-xl border border-white/10 p-5 sm:p-8 rounded-2xl sm:rounded-3xl w-full max-w-md shadow-2xl">
        ...
        {/* 💥 FIXED GRID (mobile safe) */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6 sm:mb-8">
          
          <div className="bg-[#27272a]/50 p-3 rounded-xl text-center">
            <p className="text-[10px] text-gray-400">Total</p>
            <p className="font-bold text-base sm:text-lg">₹{totalFee}</p>
          </div>

          <div className="bg-[#27272a]/50 p-3 rounded-xl text-center">
            <p className="text-[10px] text-gray-400">Paid</p>
            <p className="font-bold text-base sm:text-lg text-emerald-400">₹{totalPaid}</p>
          </div>

          <div className="bg-red-500/10 p-3 rounded-xl text-center">
            <p className="text-[10px] text-red-300">Due</p>
            <p className="font-bold text-base sm:text-lg text-red-400">₹{remaining}</p>
          </div>
        </div>
        ...
        <div className="space-y-4 sm:space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-3 py-3 sm:py-4 bg-[#27272a] rounded-xl text-sm outline-none"
              />
            </div>
          </div>

          <div>
            <label className="text-[10px] text-gray-400 mb-1 block">Remark</label>
            <div className="relative">
              <div className="absolute left-3 top-3">
                <NoteIcon />
              </div>
              <textarea
                ...
                className="w-full pl-10 pr-3 py-3 sm:py-4 bg-[#27272a] rounded-xl text-sm outline-none min-h-[90px]"
              />
            </div>
          </div>
          ...
          <button
            onClick={handleSubmit}
            className="w-full bg-emerald-500 py-3 sm:py-4 rounded-xl font-bold text-sm sm:text-lg active:scale-95"
          >
            Collect Payment
          </button>
        </div>
