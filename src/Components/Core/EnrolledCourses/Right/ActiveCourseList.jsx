...
const ActiveCourseList = () => {
  ...
  if (loading)
    return (
      <div className="h-screen bg-[#000000] flex flex-col items-center justify-center gap-4 px-4">
        <div className="w-10 h-10 sm:w-12 sm:h-12 border-4 border-[#6A0DAD]/20 border-t-[#6A0DAD] rounded-full animate-spin" />
        <p className="text-[10px] font-bold uppercase tracking-[0.25em] sm:tracking-[0.4em] text-[#4b5563] text-center">
  ...
  return (
    <div className="min-h-screen bg-[#000000] text-[#ffffff] px-4 sm:px-6 md:px-10 py-6 sm:py-8 md:py-10 font-sans">
      <div className="max-w-7xl mx-auto flex flex-col gap-8 sm:gap-10">

        {/* HEADER */}
        <div className="flex flex-col gap-4 border-b border-[#ffffff]/5 pb-6 sm:pb-10">
          
          {/* Back */}
          <div
            className="flex items-center gap-2 text-[#4b5563] hover:text-[#10b981] cursor-pointer w-fit"
            onClick={() => navigate("/dashboard/my-profile")}
          >
            <FiArrowLeft />
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-3">
            ...
            {/* Show on all devices (was hidden on mobile) */}
            <div className="flex items-center gap-2 px-3 py-2 bg-[#ffffff]/5 border border-[#ffffff]/10 rounded-xl w-fit">
              <FiZap className="text-[#10b981]" />
              <span className="text-[10px] font-bold uppercase tracking-widest">
                {userBuyedCoursesDataForCard?.courses?.length || 0} Live
              </span>
            </div>
          </div>
        </div>
        ...
        {userBuyedCoursesDataForCard?.courses?.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 sm:py-32 gap-4 border border-dashed border-[#ffffff]/10 rounded-3xl sm:rounded-[3rem] text-center px-4">
            
            <FiInbox size={40} className="text-[#4b5563]" />

            <div>
              <p className="text-base sm:text-lg font-bold uppercase tracking-tight">
                No Active Enrollments
              </p>
              <p className="text-[10px] sm:text-xs text-[#4b5563] mt-1 uppercase tracking-widest font-bold">
                Start by exploring the store
              </p>
            </div>

            <button
              onClick={() => navigate("/courses")}
              className="mt-3 sm:mt-4 px-6 sm:px-8 py-2.5 sm:py-3 bg-white text-black font-bold rounded-xl text-[10px] uppercase tracking-widest hover:bg-[#10b981] active:scale-95 transition-all"
            >
              Visit Store
            </button>
          </div>
