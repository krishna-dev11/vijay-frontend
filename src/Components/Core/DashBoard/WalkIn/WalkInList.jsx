...
const WalkInList = () => {
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#000] text-white p-4 md:p-12 font-sans overflow-x-hidden">
      
      {/* ─── Background Branding (Hidden on Mobile) ─── */}
      <div className="absolute top-[10%] left-[-5%] select-none pointer-events-none opacity-[0.02] z-0 hidden lg:block">
        <h1 className="text-[15rem] font-bold uppercase tracking-widest leading-none">Inquiry</h1>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto flex flex-col gap-6 md:gap-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-white/5 pb-8 md:pb-10">
          <div className="w-full">
            ...
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight">Walk-In Pipeline</h2>
          </div>
          <button 
            onClick={() => navigate("/dashboard/walkin/add")}
            className="w-full md:w-auto px-8 py-4 bg-white text-black font-bold rounded-2xl shadow-[0_0_40px_rgba(255,255,255,0.1)] hover:bg-gray-200 transition-all text-[10px] md:text-xs uppercase tracking-widest active:scale-95 shrink-0"
          >
            + Register Inquiry
          </button>
        </div>
        ...
        <div className="w-full space-y-4">
          ...
          <div className="hidden md:grid grid-cols-12 gap-4 px-8 py-4 bg-white/[0.03] border-x border-t border-white/10 rounded-t-[2rem] text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500">
            <div className="col-span-3">Student Profile</div>
            <div className="col-span-2">Lead Aging</div>
            <div className="col-span-2">Target Batch</div>
            ...
            {walkIns?.length > 0 ? (
              walkIns.map((w, index) => {
                ...
                return (
                  <div 
                    key={w._id} 
                    className={`bg-white/[0.02] md:bg-transparent border border-white/10 md:border-x md:border-b md:border-t-0 md:rounded-none rounded-2xl hover:bg-white/[0.04] transition-all group overflow-hidden`}
                  >
                    <div className="grid grid-cols-1 md:grid-cols-12 items-center gap-4 md:gap-4 p-6 md:p-8">
                      
                      {/* 1. Profile */}
                      <div className="md:col-span-3 flex items-center gap-4">
                        <div className="w-12 h-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 group-hover:text-emerald-400 transition-all shrink-0">
                          <FiUser size={20} />
                        </div>
                        <div className="min-w-0">
                          <p className="text-sm font-bold text-white uppercase tracking-tight truncate">{w.studentName}</p>
                          <div className="flex flex-col mt-1 space-y-0.5">
                            <span className="flex items-center gap-1.5 text-[9px] text-gray-500 font-bold uppercase">
                              <FiPhone className="text-emerald-500/50" size={10} /> {w.phone}
                            </span>
                            <span className="flex items-center gap-1.5 text-[9px] text-gray-500 font-bold lowercase truncate">
                              <FiMail className="text-emerald-500/50" size={10} /> {w.email}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      {/* 2. Aging */}
                      <div className="md:col-span-2 border-t border-white/5 pt-4 md:border-none md:pt-0">
                        <div className="flex flex-col gap-1.5 w-full md:w-28">
                          <div className="flex justify-between items-center md:block">
                            <span className="md:hidden text-[8px] font-bold text-gray-600 uppercase">Aging Status</span>
                            <span className={`text-[10px] font-mono font-bold ${days > 7 ? 'text-red-400' : 'text-emerald-400'}`}>
                              {days}D ACTIVE
                            </span>
                          </div>
                      ...
                      {/* 3. Batch */}
                      <div className="md:col-span-2 border-t border-white/5 pt-4 md:border-none md:pt-0">
                         <span className="md:hidden text-[8px] font-bold text-gray-600 uppercase block mb-1">Target Node</span>
                      ...
                      <div className="md:col-span-2 border-t border-white/5 pt-4 md:border-none md:pt-0">
                        <div className="flex flex-col gap-2">
                          <p className="text-[10px] text-gray-500 font-bold uppercase flex items-center gap-2">
                            <FiCalendar className="text-gray-700" size={12} /> {FormateDate(w.createdAt)}
                          </p>
                          <p className="text-[10px] text-emerald-500/80 font-bold uppercase flex items-center gap-2">
                            <FiActivity size={12} /> {w.followUpDate ? FormateDate(w.followUpDate) : "TBD"}
                          </p>
                        </div>
                      </div>
                      ...
                      {/* 6. Actions */}
                      <div className="md:col-span-1 border-t border-white/5 pt-4 md:border-none md:pt-0 flex items-center justify-end gap-3">
                        <button 
                          onClick={() => navigate(`/dashboard/walkin/convert/${w._id}`)}
                          className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-500 text-black hover:bg-emerald-400 transition-all text-[10px] font-bold uppercase tracking-widest"
                        >
                          <FiTrendingUp size={14} /> Enroll
                        </button>
                        
                        {days > 7 && w.status !== "Enrolled" && (
                          <button 
                            onClick={() => navigate(`/dashboard/walkin/mark-not-interested/${w._id}`)}
                            className="p-2.5 rounded-xl border border-red-500/20 text-red-500/50 hover:bg-red-500 hover:text-white transition-all shrink-0"
                            title="Archive Inquiry"
                          >
                            <FiXCircle size={18} />
                          </button>
              ...
              <div className="p-20 text-center bg-white/[0.02] border border-dashed border-white/10 rounded-[2.5rem]">
                <FiActivity className="mx-auto text-gray-800 mb-4" size={40} />
                <p className="text-gray-600 font-bold uppercase tracking-[0.3em] text-[10px]">No active pipeline data</p>
              </div>
            )}
          </div>
        </div>
