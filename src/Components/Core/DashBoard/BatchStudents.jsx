...
import { 
  FiArrowLeft, FiPhone, FiMail, FiCalendar, 
  FiClock, FiUsers, FiActivity, FiZap, FiDollarSign 
} from "react-icons/fi";
import { FormateDate } from "../../../Utilities/FormateDate";
...
const BatchStudents = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-[#000000] text-[#ffffff] font-sans selection:bg-[#6A0DAD]/30">
      
      {/* HEADER SECTION */}
      <div className="max-w-7xl mx-auto p-4 md:p-10 space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-[#ffffff]/5 pb-8">
          <div className="space-y-2 w-full">
            <nav className="flex items-center gap-2 text-[9px] md:text-[10px] font-bold uppercase tracking-[0.3em] text-[#4b5563]">
              <span>Academy Hub</span> 
              <span className="text-[#10b981]">/</span>
              <span className="text-[#ffffff]">Batch Registry</span>
            </nav>
            ...
            <p className="text-[#10b981] text-[10px] font-bold uppercase tracking-[0.2em] mt-2">
              {batch.isOfflineBatch ? "Offline Deployment" : "Online Node"}
            </p>
          </div>
          ...
          <button
            onClick={() => navigate(-1)}
            className="flex items-center justify-center gap-2 bg-[#ffffff]/5 border border-[#ffffff]/10 px-6 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-[#ffffff]/10 hover:border-[#ffffff]/20 transition-all w-full md:w-auto"
          >
            <FiArrowLeft /> Back to Hub
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {metaItems.map((item, i) => (
            <div key={i} className="bg-[#ffffff]/[0.02] border border-[#ffffff]/5 p-4 rounded-2xl hover:border-[#ffffff]/10 transition-colors group">
              <div className="flex items-center gap-2 text-[#4b5563] group-hover:text-[#ffffff]/40 transition-colors">
                <item.icon size={12} />
                <span className="text-[8px] font-bold uppercase tracking-widest">{item.label}</span>
              </div>
              <p className="text-xs md:text-sm font-bold mt-2 truncate" style={{ color: item.color || "#ffffff" }}>
                {item.val}
              </p>
            </div>
          ))}
        ...
        <div className="w-full">
          ...
          <div className="hidden md:grid grid-cols-12 gap-4 px-6 py-4 bg-[#ffffff]/[0.03] rounded-t-3xl border-x border-t border-[#ffffff]/10 text-[10px] font-bold uppercase tracking-[0.2em] text-[#4b5563]">
            <div className="col-span-4">Candidate</div>
            <div className="col-span-3">Contact Node</div>
            <div className="col-span-2">Joined</div>
            <div className="col-span-3">Financial Protocol</div>
          </div>
          ...
          <div className="flex flex-col gap-4 md:gap-0">
            {batch.studentEnrolled?.length > 0 ? (
              batch.studentEnrolled.map((entry, index) => {
                ...
                const enrollment = entry.enrollment;
                const totalFee = enrollment?.totalFee || batch.price || 0;
                const paid = enrollment?.amountPaidSoFar || 0;
                const remaining = totalFee - paid;

                return (
                  <div 
                    key={index} 
                    className="bg-[#ffffff]/[0.02] md:bg-transparent border border-[#ffffff]/10 md:border-x md:border-b md:border-t-0 md:rounded-none rounded-2xl overflow-hidden hover:bg-[#ffffff]/[0.03] transition-all"
                  >
                    <div className="grid grid-cols-1 md:grid-cols-12 items-center gap-4 p-5 md:p-6">
                      
                      {/* Candidate Column */}
                      <div className="md:col-span-4 flex items-center gap-4">
                        <img
                          src={student?.imageUrl}
                          className="w-12 h-12 rounded-xl border border-[#ffffff]/10 object-cover shrink-0"
                          alt="Profile"
                        />
                        <div className="flex flex-col min-w-0">
                          <p className="text-sm font-bold text-[#ffffff] uppercase tracking-tight truncate">
                            {student?.firstName} {student?.lastName}
                          </p>
                          <p className="text-[9px] text-[#4b5563] font-bold uppercase tracking-widest">
                            ID: #{index + 101}
                      ...
                      <div className="md:col-span-3 space-y-2">
                        <div className="flex items-center gap-2 text-[10px] font-medium text-[#9ca3af] break-all">
                          <FiMail className="text-[#10b981] shrink-0" size={12} />
                          {student?.email}
                        </div>
                        <div className="flex items-center gap-2 text-[10px] font-medium text-[#9ca3af]">
                          <FiPhone className="text-[#10b981] shrink-0" size={12} />
                          {student?.additionalDetails?.contactNumber || "N/A"}
                        </div>
                      </div>
                      ...
                      <div className="md:col-span-2">
                        <div className="flex md:block items-center justify-between border-t border-[#ffffff]/5 md:border-none pt-2 md:pt-0">
                          <span className="md:hidden text-[9px] font-bold text-[#4b5563] uppercase tracking-widest">Joined:</span>
                          <span className="text-[11px] font-bold text-[#9ca3af] md:text-[#4b5563] uppercase tracking-widest">
                             {enrollment?.createdAt ? FormateDate(enrollment.createdAt) : "N/A"}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="md:col-span-3">
                        <div className="bg-[#ffffff]/[0.03] p-4 rounded-2xl border border-[#ffffff]/5 space-y-3 w-full">
                          <div className="flex justify-between text-[10px] font-bold tracking-widest uppercase">
                            <span className="text-[#4b5563]">Ledger:</span>
                            <span className="text-[#ffffff]">₹{totalFee}</span>
                          </div>
                          <div className="flex justify-between text-[10px] font-bold tracking-widest uppercase">
                            <span className="text-[#4b5563]">Paid:</span>
                            <span className="text-[#10b981]">₹{paid}</span>
                          </div>
                          <div className="flex justify-between text-[10px] font-bold tracking-widest uppercase border-b border-[#ffffff]/5 pb-2">
                          ...
                          <div className="flex justify-between items-center pt-1 gap-2">
                            <span className={`px-2 py-1 rounded-md text-[8px] font-black uppercase tracking-widest 
                              ${student?.paymentStatus === "Paid" ? "bg-[#10b981]/10 text-[#10b981]" : 
                                student?.paymentStatus === "Partial" ? "bg-[#eab308]/10 text-[#eab308]" : "bg-[#ef4444]/10 text-[#ef4444]"}`}>
                              {student?.paymentStatus || "NOTPAID"}
                            </span>

                            <button
                              disabled={totalFee === paid && remaining === 0}
                              onClick={() => navigate(`/EnrolledCourses/collect-installment/${batch._id}/${student._id}`)}
                              className="bg-[#ffffff] text-[#000000] px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-[#10b981] disabled:opacity-30 disabled:hover:bg-[#ffffff] transition-all"
                            >
                              {totalFee === paid && remaining === 0 ? "Done" : "Collect"}
                            </button>
                          </div>
                        </div>
                      </div>
              ...
              <div className="p-20 text-center border border-dashed border-[#ffffff]/10 rounded-3xl">
                <p className="text-[#4b5563] text-[10px] font-bold uppercase tracking-[0.5em]">
                  Zero Candidates Synchronized
                </p>
              </div>
            )}
          </div>
