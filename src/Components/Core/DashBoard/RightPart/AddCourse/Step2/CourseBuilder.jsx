...
const CourseBuilder = () => {
  ...
  return (
    <div className="flex flex-col gap-8 max-w-5xl mx-auto pb-10 px-3 sm:px-4">
      
      {/* ─── BUILDER CANVAS ─── */}
      <div className="bg-white/[0.02] border border-white/10 p-5 sm:p-8 rounded-3xl backdrop-blur-2xl shadow-2xl relative overflow-hidden">
        ...
        {/* FORM */}
        <form onSubmit={handleSubmit(onFormSubmit)} className="mt-8 flex flex-col gap-3">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <input
              placeholder="e.g. Introduction to Public Speaking"
              {...register("sectionName", { required: true })}
              className="w-full bg-white/[0.03] border border-white/5 rounded-xl px-4 py-3 text-white text-sm focus:border-emerald-500/30 outline-none placeholder-gray-600"
            />

            <button 
              type="submit"
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-3 bg-emerald-500 text-white font-bold rounded-xl hover:bg-emerald-400 transition-all active:scale-95"
            >
              <FiPlus />
          ...
          {editSection && (
            <button
              ...
              className="text-xs text-gray-500 hover:text-white transition w-fit ml-1 underline"
            >
              Cancel Edit
            </button>
          )}
      ...
      {/* ─── NAVIGATION ─── */}
      <div className="flex flex-col sm:flex-row gap-4 sm:justify-between sm:items-center px-2 sm:px-4">
        ...
        <button 
          onClick={() => dispatch(setStep(3))}
          className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-white/5 border border-white/10 text-white font-bold rounded-xl hover:bg-white/10 text-xs uppercase tracking-widest"
        >
