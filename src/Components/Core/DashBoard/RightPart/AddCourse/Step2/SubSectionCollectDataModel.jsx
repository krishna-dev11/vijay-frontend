...
const SubSectionCollectDataModel = () => {
  ...
  const closeModal = () => {
    ...
    dispatch(SetviewSubSection(null));
  };

  const inputStyle =
    "w-full bg-white/[0.03] border border-white/5 rounded-xl px-4 py-3 text-white text-sm focus:border-emerald-500/30 outline-none";

  const labelStyle =
    "text-[11px] font-bold uppercase tracking-[0.15em] text-gray-500 mb-2 block ml-1";

  return (
    <div className="fixed inset-0 z-[2000] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-md">
      
      {/* 🔥 Modal Container */}
      <div className="w-full sm:max-w-lg bg-[#0a0a0a] border border-white/10 rounded-t-3xl sm:rounded-[2.5rem] shadow-2xl overflow-hidden animate-fadeSlideUp max-h-[90vh] overflow-y-auto">
        
        {/* Header */}
        <div className="p-4 sm:p-6 flex justify-between items-center border-b border-white/5 sticky top-0 bg-[#0a0a0a] z-10">
          <div className="flex items-center gap-2">
            <FiVideo className="text-emerald-400" />
            <h3 className="text-base sm:text-lg font-bold text-white">
              {addSubSection ? "Entry Protocol" : "Lecture Metadata"}
            </h3>
          </div>
        ...
        >
          ...
          {!viewSubSection && (
            <button
              type="submit"
              className="w-full py-3 sm:py-4 bg-white text-black font-bold rounded-xl uppercase tracking-widest text-xs hover:bg-gray-200 active:scale-95"
            >
              {editSubSection ? "Update Entry" : "Initialize Lecture"}
            </button>
          )}
