  ...
  return (
    <div className="flex flex-col gap-3 sm:gap-4">
      {course?.courseContent?.map((section) => (
        <details
          key={section._id}
          className="group bg-white/[0.02] border border-white/5 rounded-2xl sm:rounded-3xl overflow-hidden"
        >
          {/* HEADER */}
          <summary className="list-none cursor-pointer p-4 sm:p-5 flex items-center justify-between gap-3 group-open:bg-emerald-500/5">
            
            <div className="flex items-center gap-2 sm:gap-3 flex-1">
              <FiChevronDown className="text-gray-500 group-open:rotate-180 transition-transform shrink-0" />
            ...
            {/* ACTIONS */}
            <div className="flex items-center gap-2 sm:gap-4 border-l border-white/5 pl-2 sm:pl-4">
              <button
                onClick={() => dispatch(SetEditSection(section))}
                className="p-2 text-gray-500 hover:text-emerald-400"
              >
                <FiEdit3 size={16} />
              </button>
                  ...
                  setDeleteModal({
                    ...
                    btn1Onclick: () => {
                      if (section.subSections?.length > 0)
                        return toast.error("Section not empty!");
                      dispatch(
                        DeleteSection(
          ...
          {/* BODY */}
          <div className="p-4 sm:p-6 pt-2 flex flex-col gap-2">
            {section?.subSections?.map((sub) => (
              <div
                key={sub._id}
                className="flex items-center justify-between gap-2 bg-white/5 border border-white/5 p-3 sm:p-4 rounded-xl sm:rounded-2xl"
              >
                {/* LEFT */}
                <div
                  onClick={() => dispatch(SetviewSubSection(sub))}
                  className="flex items-center gap-2 sm:gap-3 flex-1 cursor-pointer overflow-hidden"
                >
                  <FiPlayCircle className="text-emerald-400 shrink-0" />
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                  <button
                    onClick={() => dispatch(SeteditSubSection(sub))}
            ...
            <button
              ...
              className="mt-2 flex items-center gap-2 text-emerald-400 text-[11px] sm:text-xs font-bold uppercase tracking-widest hover:text-emerald-300 ml-1"
            >
              <FiPlus /> New Lecture
            </button>
          </div>
